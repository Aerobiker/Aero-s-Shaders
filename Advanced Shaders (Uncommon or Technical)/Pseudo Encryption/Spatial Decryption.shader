Shader "AeroFunky/Spatial Decryption (receiver)"
{
    Properties
    {
        ...
        _ShieldBaseColor("Base Color", Color) = (.01, .41, .45, 1)

        [Space(20)]
        [PowerSlider(4)]_Specular("Specular Factor", Range(1, 500)) = 3
        [PowerSlider(4)]_SpecularPower("Specular Power", Range(0, 100)) = 10
        _DiffuseColor("Diffuse Color", Color) = (.43, .03, .39, 0)
        _SpecularColor("Specular Color", Color) = (1, 1, 1, 0)

        [Space(20)]
        _GooshCoolColor("Goosh Cool Color", Color) = (.41, .27, .36, 1)
        _GooshWarmColor("Goosh Warm Color", Color) = (.01, .567, .45, 1)
        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            float _Range;
            sampler2D _GrabTexture_Transparent_1;
            float4 _GrabTexture_Transparent_1_TexelSize;
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float4 col = tex2D(_GrabTexture_Transparent_1, (float2(shift) + 0.5f) / float2(_GrabTexture_Transparent_1_TexelSize.zw));
