Shader "AeroFunky/Lens Distort V2"
{
	Properties
	{
		...
		_DistortionFresnelOffset("Distortion Offset", float) = 0
		_Magnification("Magnification coefficient",float) = 2
		_MagnificationOffset("Magnification Offset", float) = 0
		...
		SubShader
	{
		...
		Pass
		{
			...
			float _DistortionFresnelOffset;
			float _Magnification;
			float _MagnificationOffset;
            ...
            v2f vert (appdata v)
            {
				...
				o.normal = normalize(UnityObjectToWorldNormal(v.normal)); // Get normals and set them to world
				...
				o.distortionFresnelUV = _MagnificationOffset + pow(_DistortionFresnelOffset + o.fresnelCoords * _DistortionFresnelThreshold, _DistortionFresnel); // Make it more powerful
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				float NdotV = max(0.0, dot(i.normal, i.viewDir));
				float fresnel = _MagnificationOffset + pow(_DistortionFresnelOffset + NdotV*_DistortionFresnelThreshold,_DistortionFresnel)*_Magnification;
				...
				switch (_LensDistortionMode) {
					case 1:
						...
						break;
					default:
						coords = UNITY_PROJ_COORD(ComputeGrabScreenPos(i.vertex + float4(0, 0, 0, fresnel))); // Project the coords correctly
						break;
				}
				...
				if (_ToggleChromaticAberation) {
					...
					float fresnelAbberation = pow(NdotV*_ChromaticShiftX[3], _ChromaticShiftY[3]);
				...
				col += fresnel*_ToggleDistortionFresnelObserve; // make a preview, so its easier to set the right parameters for the distortion

				if (_ToggleFresnel) {
					col += pow(NdotV*_Fresnel, _FresnelPower);
				}
